...
fileprivate extension Alamofire.URLEncoding {
    ...
    func geocoreEncode(_ urlRequest: Alamofire.URLRequestConvertible, with parameters: Parameters?) throws -> URLRequest {
...
fileprivate extension Alamofire.SessionManager {
    ...
    @discardableResult
    func requestWithParametersAndBody(
    ...
    public func promisedGET<T: GeocoreInitializableFromJSON>(
        ...
        return Promise { resolver in
            self.GET(path, parameters: parameters) {
                result in result.propagate(toFulfillment: resolver.fulfill, rejection: resolver.reject)
            }
        }
    }
    ...
    func promisedPOST<T: GeocoreInitializableFromJSON>(
        ...
        return Promise { resolver in
            self.POST(path, parameters: parameters, body: body) { result in
                result.propagate(toFulfillment: resolver.fulfill, rejection: resolver.reject)
            }
        }
    }
